...
class CategoryRepository @Inject() (dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext) {
  ...
  def update(id: Int, categoryName: String, categoryDescription: String): Future[Int] = {

    val q = category.filter(_.categoryID === id)
      .map(x => (x.categoryName, x.categoryDescription))
